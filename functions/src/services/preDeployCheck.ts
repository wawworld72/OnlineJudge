...
/**
 * FR-007. 데이터를 변경하지 않고 판정만 한다. Classroom 두 항목은 User Story 5가 필수임을
 * 인지시키는 목적일 뿐, 급한 사정으로 Classroom 없이 진행해야 하는 경우를 막지 않기 위해
 * 항상 WARN 이하로만 판정한다(FR-007, 절대 BLOCK 아님). `setQuizStatus`가 `OPEN` 전환을
 * 막을지 판단할 때도 이 함수를 그대로 재사용해, 저장된 이전 점검 결과가 최신 데이터와
 * 어긋나는 문제를 피한다.
 */
    ...
    items.push(
      rosterSnap.empty
        ? {
            ...
            level: "WARN",
            message: "수강생 명단이 아직 동기화되지 않았습니다.",
          }

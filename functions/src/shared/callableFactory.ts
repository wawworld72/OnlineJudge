 ...
 * 전 Zod 스키마로 입력을 검증한다. 로그인하지 않은 호출도 여기서 즉시 거부한다.
 */
export function createCallable<T, R>(schema: ZodType<T>, handler: Handler<T, R>) {
  return onCall<unknown>({ enforceAppCheck: true }, async (request: CallableRequest<unknown>) => {

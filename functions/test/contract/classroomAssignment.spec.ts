...
const { deployClassroomAssignment, resetClassroomDeployment } = await import(
  "../../src/callable/classroomAssignment"
);
...
describe("deployClassroomAssignment / resetClassroomDeployment", () => {
  ...
  it("배포 전 점검을 통과한 퀴즈를 배포하면 courseWorkId/Link를 저장한다", async () => {
    ...
    const response = await deployClassroomAssignment.run(
  ...
  it("이미 배포된 퀴즈의 재배포 요청을 거부한다", async () => {
    ...
    await expect(
      deployClassroomAssignment.run(makeRequest({ quizId: QUIZ_ID }, TEACHER_EMAIL)),
  ...
  it("resetClassroomDeployment 이후에는 다시 배포할 수 있다", async () => {
    ...
    await resetClassroomDeployment.run(makeRequest({ quizId: QUIZ_ID }, TEACHER_EMAIL));

...
describe("setQuizStatus", () => {
  ...
  it("차단 항목이 있으면 OPEN 전환을 거부한다", async () => {
    await expect(
      setQuizStatus.run(makeRequest({ quizId: QUIZ_ID, status: "OPEN" }, TEACHER_EMAIL)),
  ...
  it("차단 항목이 없으면 OPEN으로 전환된다", async () => {
    ...
    const response = await setQuizStatus.run(
      makeRequest({ quizId: QUIZ_ID, status: "OPEN" }, TEACHER_EMAIL),
    );
  ...
  it("CLOSED로 전환할 때는 배포 전 점검을 요구하지 않는다", async () => {
    const response = await setQuizStatus.run(
      makeRequest({ quizId: QUIZ_ID, status: "CLOSED" }, TEACHER_EMAIL),
    );

...
async function seedQuiz(archivedAt: Timestamp | null) {
  ...
  await db.collection("participants").doc(`${QUIZ_ID}_20240001`).set({
    quizId: QUIZ_ID,
    studentId: "20240001",
...
describe("deleteQuizData", () => {
  ...
  it("아카이브가 없으면 NOT_ARCHIVED_YET으로 거부한다", async () => {
    ...
    await expect(deleteQuizData.run(makeTeacherRequest({ quizId: QUIZ_ID }))).rejects.toMatchObject(
      { details: { code: "NOT_ARCHIVED_YET" } },
    );
  });
  ...
  it("confirmWithoutArchive: true면 아카이브 없이도 삭제를 강행한다", async () => {
    ...
    const response = await deleteQuizData.run(
      makeTeacherRequest({ quizId: QUIZ_ID, confirmWithoutArchive: true }),
    );
  ...
  it("아카이브된 퀴즈는 문항/테스트케이스/참가자 데이터를 모두 삭제하고 퀴즈 문서에 deletedAt만 남긴다", async () => {
    ...
    await deleteQuizData.run(makeTeacherRequest({ quizId: QUIZ_ID }));
    ...
    expect(
      (await db.collection("quizzes").doc(QUIZ_ID).collection("problemSecrets").doc("p1").get())
        .exists,
    ).toBe(false);
    expect((await db.collection("participants").doc(`${QUIZ_ID}_20240001`).get()).exists).toBe(
      false,
    );

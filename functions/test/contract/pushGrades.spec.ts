...
const { listCourseStudents, listStudentSubmissions, patchGrade } = await import(
...
async function seedFinalizedParticipant(studentId: string, finalTotal: number) {
  await testDb().collection("students").doc(studentId).set({
    name: studentId,
...
describe("pushGrades", () => {
  ...
  it("확정 채점된 참가자가 없으면 NO_FINALIZED_PARTICIPANTS로 거부한다", async () => {
    ...
    await expect(pushGrades.run(makeRequest({ quizId: QUIZ_ID }, TEACHER_EMAIL))).rejects.toMatchObject(
  ...
  it("학번-이메일-Classroom 사용자-제출물 연결이 끊긴 학생은 건너뛰고 실패로 집계한다", async () => {
    ...
    const response = await pushGrades.run(makeRequest({ quizId: QUIZ_ID }, TEACHER_EMAIL));

...
const { listCourseStudents, createCourseWork, listStudentSubmissions, patchGrade } =
...
describe("교사의 Classroom 연동 흐름 (수강생 동기화 → 과제 배포 → 성적 반영)", () => {
  ...
  it("수강생 동기화 → 과제 배포 → 성적 반영을 순서대로 실행한다", async () => {
    ...
    const syncResult = await syncRoster.run(makeTeacherRequest({ courseId: COURSE_ID }));
    ...
    const deployResult = await deployClassroomAssignment.run(
      makeTeacherRequest({ quizId: QUIZ_ID }),
    );
    expect(deployResult.courseWorkId).toBe("cw-1");

    await testDb().collection("participants").doc(`${QUIZ_ID}_20240001`).set({
      quizId: QUIZ_ID,
      studentId: "20240001",
    ...
    const pushResult = await pushGrades.run(makeTeacherRequest({ quizId: QUIZ_ID }));

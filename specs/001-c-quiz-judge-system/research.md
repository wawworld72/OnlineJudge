...
## 21. `syncRoster`와 학생 이메일 불변 규칙(FR-012)의 관계 — 구현 중 명확화한 항목

- **Decision**: `syncRoster`는 Classroom이 이미 알고 있는 학생 이메일을 신뢰된 출처로
  간주해 `students/{studentId}.email`을 최신 값으로 덮어쓸 수 있다 — 이미 이메일이
  등록되어 있어도 막지 않는다. FR-012의 "재등록 거부"는 `registerStudentEmail`(학생이
  스스로 호출하는 셀프서비스 경로)에만 적용되는 규칙이다.
- **Rationale**: `syncRoster`(FR-026)는 "학생명부와 분반-학생 매핑을 최신화"하는 것이
  목적이고, Classroom 계정은 이미 학교가 발급·관리하는 이메일이므로 학생이 스스로 등록할
  필요가 있는 정보가 아니다. FR-011/012는 애초에 "Classroom 계정 이메일을 아직 모르는
  상태에서 학생이 최초 입장할 때"를 다루는 보완 경로다 — 두 FR을 같은 함수의 같은 규칙으로
  묶으면 Classroom이 이미 동기화한 분반에서도 교사가 학생 이메일을 고칠 방법이 없어지는
  모순이 생긴다. `syncRoster`(교사 트리거, Classroom이 출처)와
  `registerStudentEmail`(학생 트리거, 셀프서비스)을 서로 다른 신뢰 경로로 구분해 이
  모순을 해소했다.

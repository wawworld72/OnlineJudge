...
**배점 합계 재계산은 항상 트랜잭션 내부에서 재조회한 배열을 기준으로 한다**: 클라이언트는
`items` 전체 배열이나 배점 합계를 절대 보내지 않고, 테스트케이스 1개 단위의 델타(추가/치환/
제거 대상 1건)만 보낸다. 서버는 `tx.get()`으로 그 순간의 `problemSecrets.items`를 트랜잭션
안에서 다시 읽은 뒤 그 델타를 적용하고, 그 결과 배열로 배점 합을 계산한다. 이렇게 해야 교사가
같은 문항을 두 탭에서 동시에 편집해도(각 탭이 서로 다른 테스트케이스를 수정) Firestore
트랜잭션의 자동 재시도(커밋 시점에 읽은 문서가 그사이 바뀌었으면 최신 값으로 재시도)에 의해
두 수정이 모두 최종 배점 합계에 반영된다 — 클라이언트가 계산한 배열/합계를 그대로 믿는
방식이었다면, 나중에 커밋되는 수정이 먼저 커밋된 수정을 덮어써 배점 합계에서 누락되는
